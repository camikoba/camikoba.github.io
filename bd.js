...
function pesquisaPorNome() {
    const nomeUsuario = document.getElementById("pesquisa-nome-usuario").value.toUpperCase();
    if (nomeUsuario === ""){
    ...
    bd.transaction(function (ler) {
        ler.executeSql(
            ...
            function (ler, resultados) {
                ...
                document.getElementById("resultado-pesquisa").value = nome +", " + idade +" anos";
            }    
        );
    });
}
...
function exibeBD() {
    bd.transaction(function (exibe) {
        exibe.executeSql("SELECT * FROM formulario", [], 
